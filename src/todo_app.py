...
class App_todo():
    ...
    def register(self):
        usuari_nou = User(9999,self.usernameEntry2.get(),self.emailentry.get(),self.passwordEntry2.get())
        res = self.controller_user.add(usuari_nou)
        
        if(type(res)==int):
            self.message_label.config(text="Invalid username or password")
        else:
            self.message_label.config(text="")
            self.id_user = self.controller_user.get_by_username(self.usernameEntry2.get()).id
            self.log_tk.destroy()
            self.open_calendar()
